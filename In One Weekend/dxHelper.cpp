...
ComPtr<IDXGIAdapter> getRTXAdapter()
{
	if (!gAdapter_v0)
	{
		for (uint i = 0; DXGI_ERROR_NOT_FOUND != getFactory()->EnumAdapters(i, &gAdapter_v0); i++)
		{
			...
			if (FAILED(hr))
			{
				throw Error("Your Windows 10 version must be at least 1809 to support D3D12_FEATURE_D3D12_OPTIONS5.");
			}
		...
		throw Error("There are not DXR supported adapters(video cards suchs as Nvidia's Volta or Turing RTX).Also, The DXR fallback layer is not supported int this application.");
	}
