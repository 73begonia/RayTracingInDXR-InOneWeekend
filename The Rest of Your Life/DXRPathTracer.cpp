...
void DXRPathTracer::declareRootSignatures()
{
	...
	localRootParams[HitGroupParamID::constantForObject].Constants.Num32BitValues = (sizeof(ObjectContants) + 3) / 4;
...
void DXRPathTracer::buildRaytracingPipeline()
{
	...
	vector<D3D12_HIT_GROUP_DESC> hitGroupDesc;
	hitGroupDesc.resize(2);
	hitGroupDesc[0].Type = D3D12_HIT_GROUP_TYPE_TRIANGLES;
	hitGroupDesc[0].ClosestHitShaderImport = cClosestHitShaderName;
	hitGroupDesc[0].AnyHitShaderImport = nullptr;
	hitGroupDesc[0].HitGroupExport = cHitGroupName;
	hitGroupDesc[0].IntersectionShaderImport = nullptr;
	subObjHitGroup.pDesc = (void*)&hitGroupDesc[0];
	subObjHitGroup.Type = D3D12_STATE_SUBOBJECT_TYPE_HIT_GROUP;

	subObjects[index++] = subObjHitGroup;

	//2
	D3D12_STATE_SUBOBJECT subObjHitGroupGlass;

	hitGroupDesc[1].Type = D3D12_HIT_GROUP_TYPE_TRIANGLES;
	hitGroupDesc[1].ClosestHitShaderImport = cClosesthitGlassShaderName;
	hitGroupDesc[1].AnyHitShaderImport = nullptr;
	hitGroupDesc[1].HitGroupExport = cHitGroupGlassName;
	hitGroupDesc[1].IntersectionShaderImport = nullptr;
	subObjHitGroupGlass.pDesc = (void*)&hitGroupDesc[1];
	subObjHitGroupGlass.Type = D3D12_STATE_SUBOBJECT_TYPE_HIT_GROUP;

	subObjects[index++] = subObjHitGroupGlass;
	...
	exportName[1] = cHitGroupGlassName;
...
TracedResult DXRPathTracer::shootRays()
{
	...
	desc.HitGroupTable.StrideInBytes = _align(D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES + (uint)sizeof(ObjectContants), D3D12_RAYTRACING_SHADER_RECORD_BYTE_ALIGNMENT);
...
void DXRPathTracer::setupShaderTable()
{
	...
	void* pHitGroupGlassShaderIdentifier;
	...
	pHitGroupGlassShaderIdentifier = pStateObjectProperties->GetShaderIdentifier(cHitGroupGlassName);
	...
	//hit group shader table
	{
		...
		uint nShaderRecordSize = D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES + sizeof(ObjectContants);
		...
		for (uint i = 0; i < numObjs; ++i)
		{
			...
			memcpy(pBufs + nShaderRecordSize * i + D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES, &objConsts[i], sizeof(ObjectContants));
		}
...
//{
//	box = loadMeshFromOBJFile("../__data/mesh/lucy.obj", true);
//
//	mVertexBuffer = createCommittedBuffer(sizeof(Vertex) * box.vtxArr.size());
//	uint8* pVertexData;
//	mVertexBuffer->Map(0, nullptr, (void**)&pVertexData);
//	memcpy(pVertexData, box.vtxArr.data(), static_cast<uint>(sizeof(Vertex) * box.vtxArr.size()));
//	mVertexBuffer->Unmap(0, nullptr);
//
//	mIndexBuffer = createCommittedBuffer(sizeof(Tridex) * box.tdxArr.size());
//	uint8* pIndexData;
//	mIndexBuffer->Map(0, nullptr, (void**)&pIndexData);
//	memcpy(pIndexData, box.tdxArr.data(), static_cast<uint>(sizeof(Tridex) * box.tdxArr.size()));
//	mIndexBuffer->Unmap(0, nullptr);
//
//	//bottom
//	D3D12_RAYTRACING_GEOMETRY_DESC geomDesc = {};
//	geomDesc.Type = D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES;
//	geomDesc.Triangles.VertexBuffer.StartAddress = mVertexBuffer->GetGPUVirtualAddress();
//	geomDesc.Triangles.VertexBuffer.StrideInBytes = sizeof(Vertex);
//	geomDesc.Triangles.VertexFormat = DXGI_FORMAT_R32G32B32_FLOAT;
//	geomDesc.Triangles.VertexCount = (uint)box.vtxArr.size();
//	geomDesc.Triangles.IndexBuffer = mIndexBuffer->GetGPUVirtualAddress();
//	geomDesc.Triangles.IndexCount = (uint)box.tdxArr.size() * 3;
...
//	mDevice_v5->GetRaytracingAccelerationStructurePrebuildInfo(&bottomLevelInputs, &bottomLevelInfo);
//	ThrowIfFailed(bottomLevelInfo.ResultDataMaxSizeInBytes > 0);
//
//	
//	bottomScratch = createCommittedBuffer(
//		bottomLevelInfo.ScratchDataSizeInBytes,
//		D3D12_HEAP_TYPE_DEFAULT,
//		D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS,
//		D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
//	
//	bottomResult = createCommittedBuffer(
//		bottomLevelInfo.ResultDataMaxSizeInBytes,
//		D3D12_HEAP_TYPE_DEFAULT,
//		D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS,
//		D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE);
//
//	D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC bottomLevelAsDesc = {};
//	bottomLevelAsDesc.Inputs = bottomLevelInputs;
//	bottomLevelAsDesc.DestAccelerationStructureData = bottomResult->GetGPUVirtualAddress();
//	bottomLevelAsDesc.ScratchAccelerationStructureData = bottomScratch->GetGPUVirtualAddress();
//
//	mCmdList_v4->BuildRaytracingAccelerationStructure(&bottomLevelAsDesc, 0, nullptr);
//
//	D3D12_RESOURCE_BARRIER bottomLevelUAVBarrier = {};
//	bottomLevelUAVBarrier.Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
//	bottomLevelUAVBarrier.UAV.pResource = bottomResult.Get();
//	mCmdList_v4->ResourceBarrier(1, &bottomLevelUAVBarrier);
//
//	mBottomLevelAccelerationStructure.push_back(bottomResult);
//
//	//top
...
//
//	D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO topLevelInfo;
//	mDevice_v5->GetRaytracingAccelerationStructurePrebuildInfo(&topLevelInputs, &topLevelInfo);
//	ThrowIfFailed(topLevelInfo.ResultDataMaxSizeInBytes > 0);
//
//	
//	topScratch = createCommittedBuffer(
//		topLevelInfo.ScratchDataSizeInBytes,
//		D3D12_HEAP_TYPE_DEFAULT,
//		D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS,
//		D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
//
//	
//	topResult = createCommittedBuffer(
//		topLevelInfo.ResultDataMaxSizeInBytes,
//		D3D12_HEAP_TYPE_DEFAULT,
//		D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS,
//		D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE);
//
//	
//	topInstanceDesc = createCommittedBuffer(
...
//	topInstanceDesc->Map(0, nullptr, (void**)&pInstanceDesc);
//
//	pInstanceDesc->InstanceID = 0;
//	pInstanceDesc->InstanceContributionToHitGroupIndex = 0;
//	pInstanceDesc->Flags = D3D12_RAYTRACING_INSTANCE_FLAG_NONE;
//
//	memcpy(pInstanceDesc->Transform, &IdentityMatrix4x4(), sizeof(pInstanceDesc->Transform));
//	pInstanceDesc->AccelerationStructure = mBottomLevelAccelerationStructure[0]->GetGPUVirtualAddress();
//	pInstanceDesc->InstanceMask = 0xFF;
//
//	topInstanceDesc->Unmap(0, nullptr);
//
//	D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC topLevelAsDesc = {};
//	topLevelAsDesc.Inputs = topLevelInputs;
//	topLevelAsDesc.Inputs.InstanceDescs = topInstanceDesc->GetGPUVirtualAddress();
//	topLevelAsDesc.DestAccelerationStructureData = topResult->GetGPUVirtualAddress();
//	topLevelAsDesc.ScratchAccelerationStructureData = topScratch->GetGPUVirtualAddress();
//
//	mCmdList_v4->BuildRaytracingAccelerationStructure(&topLevelAsDesc, 0, nullptr);
//
//	D3D12_RESOURCE_BARRIER topLevelUAVBarrier = {};
//	topLevelUAVBarrier.Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
//	topLevelUAVBarrier.UAV.pResource = topResult.Get();
//	mCmdList_v4->ResourceBarrier(1, &topLevelUAVBarrier);
//
//	mBottomLevelAccelerationStructure[0] = bottomResult;
//	mTopLevelAccelerationStructure = topResult;
//
//	mFence_v0.waitCommandQueue(mCmdQueue_v0.Get());
//}
...
void DXRPathTracer::buildAccelerationStructure()
{
	...
	numObjs = (uint)gpuMeshArr.size();

	vector<D3D12_GPU_VIRTUAL_ADDRESS> transformAddressArr(numObjs, 0);
	...
	{
		buildBLAS(&mBottomLevelAccelerationStructure[i], &Scratch[i], &gpuMeshArr[i], &transformAddressArr[i], numObjsPerBlas, sizeof(Vertex), D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_NONE);
	}
	...
	buildTLAS(&mTopLevelAccelerationStructure, &Scratch[numBottomLevels], &InstanceDescs, mBottomLevelAccelerationStructure[0].GetAddressOf(), &(*topLevelTransform)[0], numBottomLevels, 1, D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_NONE);
