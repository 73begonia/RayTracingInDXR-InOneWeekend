...
void DXRPathTracer::declareRootSignatures()
{
	...
	globalRange[1].NumDescriptors = 2;
...
void DXRPathTracer::update()
{
	...
	mGlobalConstants.backgroundLight = float3(0.0, 0.0, 0.2);
...
TracedResult DXRPathTracer::shootRays()
{
	...
	objGeometryInputsHandle.ptr += ((uint)DescriptorID::vertexBuff) * mSrvDescriptorSize;
...
{
	sphere = generateSphereMesh(float3(0), 1.f);

	uint64 vtxBuffSize = sphere.vtxArr.size() * sizeof(Vertex);
	uint64 tdxBuffSize = sphere.tdxArr.size() * sizeof(Tridex);

	ComPtr<ID3D12Resource> uploader = createCommittedBuffer(
		vtxBuffSize + tdxBuffSize);
	...
	initBuffer(mVertexBuffer, vtxBuffSize, (void*)sphere.vtxArr.data());
	initBuffer(mIndexBuffer, tdxBuffSize, (void*)sphere.tdxArr.data());
	...
	//Vertex srv
	{
		...
		srvDesc.Buffer.NumElements = (uint)sphere.vtxArr.size();
	...
	//Index srv
	{
		...
		srvDesc.Buffer.NumElements = (uint)sphere.tdxArr.size();
	...
	geomDesc.Triangles.VertexCount = (uint)sphere.vtxArr.size();
	geomDesc.Triangles.IndexBuffer = mIndexBuffer->GetGPUVirtualAddress();
	geomDesc.Triangles.IndexCount = (uint)sphere.tdxArr.size() * 3;
