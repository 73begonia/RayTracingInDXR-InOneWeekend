...
Scene* SceneLoader::push_RayTracingInOneWeekend()
{
	...
	Mesh sphere1 = generateSphereMesh(float3(0, 1, 0), 1.f);
	Mesh sphere2 = generateSphereMesh(float3(-4, 1, 0), 1.f);
	Mesh sphere3 = generateSphereMesh(float3(4, 1, 0), 1.f);
	...
	//sphere1
	Material sphere1Mtl;
	sphere1Mtl.type = MaterialType::Dielectric;
	sphere1Mtl.refractionIndex = 1.5f;
	mtlArr.push_back(sphere1Mtl);

	//sphere2
	Material sphere2Mtl;
	sphere2Mtl.type = MaterialType::Lambertian;
	sphere2Mtl.albedo = float3(0.4, 0.2, 0.1);
	mtlArr.push_back(sphere2Mtl);

	//sphere3
	Material sphere3Mtl;
	sphere3Mtl.type = MaterialType::Metal;
	sphere3Mtl.albedo = float3(0.7, 0.6, 0.5);
	sphere3Mtl.fuzz = 0.0f;
	mtlArr.push_back(sphere3Mtl);
