...
Scene* SceneLoader::push_simpleSphere()
{
	...
	Mesh sphere2 = generateSphereMesh(float3(-1, 0.001, 1), 0.5f);
	Mesh sphere3 = generateSphereMesh(float3(1, 0.001, 1), 0.5f);

	initializeGeometryFromMeshes(scene, { &ground, &sphere1, &sphere2, &sphere3 });
	...
	mtlArr[1].albedo = float3(0.7, 0.3, 0.3);

	//sphere2
	mtlArr[2].type = MaterialType::Metal;
	mtlArr[2].albedo = float3(0.8, 0.8, 0.8);
	mtlArr[2].fuzz = 0.5;
